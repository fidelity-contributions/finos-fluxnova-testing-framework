package org.finos.fluxnova.bpm.test.data.guard

import org.finos.fluxnova.bpm.test.data.FluxnovaBpmData.stringVariable
import org.finos.fluxnova.bpm.test.data.guard.condition.*
...
class FluxnovaBpmDataGuardsTest {
  ...
  fun `should construct exists condition`() {
    val condition = FluxnovaBpmDataGuards.exists(FOO)
  ...
  fun `should construct exists local condition`() {
    val condition = FluxnovaBpmDataGuards.existsLocal(FOO)
    assertThat(condition).isInstanceOf(VariableExistsGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
  ...
  fun `should construct notExists condition`() {
    val condition = FluxnovaBpmDataGuards.notExists(FOO)
  ...
  fun `should construct notExists local condition`() {
    val condition = FluxnovaBpmDataGuards.notExistsLocal(FOO)
    assertThat(condition).isInstanceOf(VariableNotExistsGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
    assertThat(condition.local).isEqualTo(true)
  ...
  fun `should construct hasValue condition`() {
    val condition = FluxnovaBpmDataGuards.hasValue(FOO, "val")
    ...
    assertThat(condition.evaluate(Optional.empty())).contains(
      GuardViolation(FluxnovaBpmDataGuards.exists(FOO), Optional.empty(), "Expecting variable 'foo' to be set, but it was not found.")
    )
  ...
  fun `should construct hasValue local condition`() {
    val condition = FluxnovaBpmDataGuards.hasValueLocal(FOO, "valLocal")
    assertThat(condition).isInstanceOf(VariableValueGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
    assertThat(condition.local).isEqualTo(true)
    ...
    assertThat(condition.evaluate(Optional.empty())).contains(
      GuardViolation(FluxnovaBpmDataGuards.existsLocal(FOO), Optional.empty(), "Expecting local variable 'foo' to be set, but it was not found.")
    )
    assertThat(condition.evaluate(Optional.of("other"))).contains(
  ...
  fun `should construct hasOneOfValues condition`() {
    val condition = FluxnovaBpmDataGuards.hasOneOfValues(FOO, setOf("val1", "val2"))
  ...
  fun `should construct hasOneOfValues local condition`() {
    val condition = FluxnovaBpmDataGuards.hasOneOfValuesLocal(FOO, setOf("valLocal1", "valLocal2"))
    assertThat(condition).isInstanceOf(VariableValueOneOfGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
    assertThat(condition.local).isEqualTo(true)
  ...
  fun `should construct matches condition`() {
    val condition = FluxnovaBpmDataGuards.matches(FOO) { it == "special_val" }
  ...
  fun `should construct matches local condition`() {
    val condition = FluxnovaBpmDataGuards.matchesLocal(FOO) { it == "special_val_local" }
    assertThat(condition).isInstanceOf(VariableMatchesGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
    assertThat(condition.local).isEqualTo(true)
  ...
  fun `should construct matches condition with validation message supplier`() {
    val condition = FluxnovaBpmDataGuards.matches(FOO, { it == "special_val" }) { variableFactory, localLabel, option ->
    ...
    assertThat(condition.evaluate(Optional.empty())).contains(
      GuardViolation(
        FluxnovaBpmDataGuards.exists(FOO),
        Optional.empty(),
  ...
  fun `should construct matches local condition with validation message supplier`() {
    val condition = FluxnovaBpmDataGuards.matchesLocal(FOO, { it == "special_val_local" }) { variableFactory, localLabel, option ->
    ...
    assertThat(condition.evaluate(Optional.empty())).contains(
      GuardViolation(
        FluxnovaBpmDataGuards.existsLocal(FOO),
        Optional.empty(),
  ...
  fun `should construct matchesRegex condition`() {
    val condition = FluxnovaBpmDataGuards.matchesRegex(FOO, "^special.*")
  ...
  fun `should construct matchesRegex local condition`() {
    val condition = FluxnovaBpmDataGuards.matchesRegexLocal(FOO, "^special.*")
    assertThat(condition).isInstanceOf(VariableMatchesRegexGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
    assertThat(condition.local).isEqualTo(true)
    assertThat(condition.evaluate(Optional.of("special_val"))).isEmpty()
  ...
  fun `should construct matchesRegex with regexDisplayName condition`() {
    val condition = FluxnovaBpmDataGuards.matchesRegex(FOO, "^special.*", "special")
    assertThat(condition).isInstanceOf(VariableMatchesRegexGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
    assertThat(condition.local).isEqualTo(false)
    assertThat(condition.evaluate(Optional.of("special_val"))).isEmpty()
  ...
  fun `should construct matchesRegex with regexDisplayName local condition`() {
    val condition = FluxnovaBpmDataGuards.matchesRegexLocal(FOO, "^special.*", "special")
    assertThat(condition).isInstanceOf(VariableMatchesRegexGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(FOO)
    assertThat(condition.local).isEqualTo(true)
    assertThat(condition.evaluate(Optional.of("special_val"))).isEmpty()
  ...
  fun `should construct matchesRegex E-Mail condition`() {
    val condition = FluxnovaBpmDataGuards.isEmail(EMAIL_VARIABLE)
    assertThat(condition).isInstanceOf(VariableMatchesRegexGuardCondition::class.java)
  ...
  fun `should construct matchesRegex E-Mail local condition`() {
    val condition = FluxnovaBpmDataGuards.isEmailLocal(EMAIL_VARIABLE)
    assertThat(condition).isInstanceOf(VariableMatchesRegexGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(EMAIL_VARIABLE)
    assertThat(condition.local).isEqualTo(true)
    assertThat(condition.evaluate(Optional.of(EMAIL))).isEmpty()
  ...
  fun `should construct matchesRegex UUID condition`() {
    val condition = FluxnovaBpmDataGuards.isUuid(UUID_VARIABLE)
    assertThat(condition).isInstanceOf(VariableMatchesRegexGuardCondition::class.java)
  ...
  fun `should construct matchesRegex UUID local condition`() {
    val condition = FluxnovaBpmDataGuards.isUuidLocal(UUID_VARIABLE)
    assertThat(condition).isInstanceOf(VariableMatchesRegexGuardCondition::class.java)
    assertThat(condition.variableFactory).isEqualTo(UUID_VARIABLE)
    assertThat(condition.local).isEqualTo(true)
    assertThat(condition.evaluate(Optional.of(MYUUID))).isEmpty()
