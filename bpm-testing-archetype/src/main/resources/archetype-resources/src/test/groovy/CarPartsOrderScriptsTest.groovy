...
class CarPartsOrderScriptsTest extends ScriptTestSpecification {
    ...
    def 'getCarPartDetails_RegularCar'() {
        ...
        when:
            executeInlineScript("GoToPartSearch", "bpmn/CarParts_Order.bpmn")
        then:
    ...
    def 'handleCarLookupResponse_noError'() {
        setup:
            ...
            mockExecutionGetVariable(variables)
        when:
            executeExternalScript("bpmn/scripts/car-lookup.groovy")
        then:
            noExceptionThrown()
    ...
    def 'ExceptionHandling'() {
        setup:
            initializeEngine(ScriptEngineType.GROOVY)
        when:
            executeInlineScript("ExceptionHandling", "bpmn/CarParts_Order.bpmn")
        then:
