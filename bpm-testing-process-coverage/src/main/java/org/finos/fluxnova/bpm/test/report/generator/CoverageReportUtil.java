package org.finos.fluxnova.bpm.test.report.generator;

import org.finos.fluxnova.bpm.test.core.export.CoverageStateJsonExporter;
...
public class CoverageReportUtil {
     ...
     */
    public static String TARGET_DIR_ROOT = System.getProperty("process-test-coverage.target-dir-root", "target/process-test-coverage/");
    ...
    private static void installReportDependencies(final File reportDirectory) {
        try {
            ...
            if (resourcesRoot.isFile()) {
                try (final JarFile coverageJar = new JarFile(resourcesRoot)) {
                    final Enumeration<JarEntry> entries = coverageJar.entries();
                    final Path reportBasePath = reportDirectory.toPath().normalize();

                    while (entries.hasMoreElements()) {
                        final String resourcePath = entries.nextElement().getName();
                        if (resourcePath.matches(REPORT_RESOURCES)) {
                            final Path targetPath = reportBasePath.resolve(resourcePath).normalize();
                            if (!targetPath.startsWith(reportBasePath)) {
                                throw new IOException("Unsafe archive entry path: " + resourcePath);
                            }

                            try (final InputStream source = CoverageReportUtil.class.getResourceAsStream("/" + resourcePath)) {
                                Files.copy(Objects.requireNonNull(source), targetPath, StandardCopyOption.REPLACE_EXISTING);
                            }
                        }
                    }
                }
